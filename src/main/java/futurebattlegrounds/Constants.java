package futurebattlegrounds;

public class Constants {
    public static final double KEEPALIVETIME = 1000; // temporarily high; will go down later!
    public static final float SHIP_ROTATE_DEG_PER_SECOND = 45.0f;
    public static final double SHIP_THRUST = 4.0;
    public static final double SHIP_SHOT_BATTERY_USAGE = 5.0;
    public static final double SHIP_SHOT_SPEED = 100.0;
    public static final double SHIP_HULL = 100;
    public static final double SHIP_BATTERY = 60;
    public static final double SHIP_BATTERY_RECHARGE_PER_SECOND = 20;
}